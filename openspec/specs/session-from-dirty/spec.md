...
Enable creating a new session by moving the current session's uncommitted (dirty) changes to a new worktree. This supports a common workflow where a developer realizes mid-work that changes should live on a separate branch, without losing any in-progress edits.
