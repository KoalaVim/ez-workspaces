...
Track and display GitHub PR status for sessions. PR metadata is populated either via the PR checkout workflow or auto-detected from the session's branch name using `gh pr list`.

## Requirements

### Requirement: PR status storage in session env
Sessions associated with a GitHub PR SHALL store PR metadata in `session.env`: `ez_pr_number` (PR number), `ez_pr_url` (full URL), and `ez_pr_status` (one of `open`, `merged`, `closed`). The status SHALL be refreshed via `gh pr view <number> --repo <remote> --json state` when the cached status is older than 5 minutes.
...
- **THEN** if a PR is found, metadata is populated and persisted
