# Rebase Branch Recovery

## Purpose

Detect active rebase state in worktrees and recover the original branch name from git's rebase state files, so users see the branch name instead of a truncated SHA or no branch when a worktree is mid-rebase.
