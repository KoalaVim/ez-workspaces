# Cursor Trusted Workspace

## Purpose

Bundled plugin that auto-creates `.workspace-trusted` files in Cursor's per-project directory for worktree sessions, eliminating the "Do you trust the authors?" prompt when opening worktrees in Cursor IDE.
